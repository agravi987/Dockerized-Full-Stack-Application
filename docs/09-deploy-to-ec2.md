# 🌤️ Milestone 9 — Deploy to EC2

## 🎯 Goal

Take the images you pushed in Milestone 8 and run them on an Amazon EC2 server ☁️, reachable from anywhere in the world. 🌍
...
```
Your images (Docker Hub) 🚢 ──pull📥──▶ EC2 server (Ubuntu + Docker) 🖥️ ──▶ http://<public-ip> 🌐
```
...
1. AWS Console → **EC2** → **Launch Instance** ☁️
...
6. **Network settings / Security group:** 🛡️
...
curl -X POST http://localhost:8080/api/messages -H "Content-Type: application/json" -d '{"name":"EC2","message":"Deployed!"}'
...
docker compose up -d                       # ▶️ restart after a crash
```
...
```
[ ] ✔️ EC2 launched (Ubuntu 24.04, t2.micro, key pair saved)
[ ] ✔️ Security group: SSH(22) to My IP, Custom TCP 8080 public
[ ] ✔️ Docker + Compose installed on the server
[ ] ✔️ Images pulled from YOUR Docker Hub account
[ ] ✔️ docker compose up -d → all Up (healthy)
[ ] ✔️ App live at http://<PUBLIC_IP>:8080
```
...
➡️ **Next:** [Milestone 10 — Domain & HTTPS](10-domain-and-https.md) *(optional)*