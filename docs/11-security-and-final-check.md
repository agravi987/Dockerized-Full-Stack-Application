# Milestone 11 — Security & Final Check *(optional)*

## Goal

Harden the server, prove the project is reproducible from scratch, and finish with a folder you're proud to show.
...
## Step 6 — The Final Test: Rebuild From Scratch
...
[ ] Containers run non-root (backend: node, frontend: nginx)
...
[ ] HEALTHCHECK on backend + frontend + db
[ ] depends_on: service_healthy for startup order
[ ] restart: unless-stopped on all services
[ ] Environment via .env (git-ignored) + .env.example (committed)
[ ] Dev overrides in docker-compose.dev.yml alone — production untouched
```

### Deployment & Security
```
[ ] Deployed to EC2 (Ubuntu 24.04)
[ ] App live via images pulled from Docker Hub
